...
class Crypter
{
    static iv = Buffer.from([0x88, 0xa6, 0xb0, 0x79, 0x5d, 0x85, 0xdb, 0xfc, 0xe6, 0xe0, 0xb3, 0xe9, 0xa6, 0x29, 0x65, 0x4b]);
    ...
    encrypt(plainText)
    {
        const cipher = crypto.createCipheriv(Crypter.algo, this.cipherKey, Crypter.iv)
        const padLength = Crypter.blockSize - (Buffer.byteLength(plainText) % Crypter.blockSize)
    ...
    decrypt(cipherText)
    {
        const decipher = crypto.createDecipheriv(Crypter.algo, this.cipherKey, Crypter.iv)
